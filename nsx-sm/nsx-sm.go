...
const (
	hipsterShopnsx-smManifestsURL      = "https://raw.githubusercontent.com/GoogleCloudPlatform/microservices-demo/master/release/nsx-sm-manifests.yaml"
...
func (iClient *Client) CreateMeshInstance(_ context.Context, k8sReq *meshes.CreateMeshInstanceRequest) (*meshes.CreateMeshInstanceResponse, error) {
	...
	if err != nil {
		err = errors.Wrapf(err, "unable to create a new nsx-sm client")
		logrus.Error(err)
		return nil, err
	}
...
func (iClient *Client) MeshName(context.Context, *meshes.MeshNameRequest) (*meshes.MeshNameResponse, error) {
	return &meshes.MeshNameResponse{Name: "nsx-sm"}, nil
}
...
func (iClient *Client) applynsx-smCRDs(ctx context.Context, delete bool) error {
...
func (iClient *Client) executeTemplate(ctx context.Context, username, namespace, templateName string) (string, error) {
	tmpl, err := template.ParseFiles(path.Join("nsx-sm", "config_templates", templateName))
	if err != nil {
...
func (iClient *Client) executeInstall(ctx context.Context, installmTLS bool, arReq *meshes.ApplyRuleRequest) error {
	arReq.Namespace = ""
	if arReq.DeleteOp {
		defer iClient.applynsx-smCRDs(ctx, arReq.DeleteOp)
	} else {
		if err := iClient.applynsx-smCRDs(ctx, arReq.DeleteOp); err != nil {
			return err
		}
	}
	yamlFileContents, err := iClient.getLatestnsx-smYAML(installmTLS)
	if err != nil {
		return err
	}
...
func (iClient *Client) executeHipsterShopInstall(ctx context.Context, arReq *meshes.ApplyRuleRequest) error {
	...
	nsx-smManifestsContent, err := hipsterShopFilecontents(hipsterShopnsx-smManifestsURL)
	if err != nil {
		return err
	}

	var yamlFileContents = fmt.Sprintf("%s\n---\n%s", kubernetesManifestsContent, nsx-smManifestsContent)
...
func (iClient *Client) ApplyOperation(ctx context.Context, arReq *meshes.ApplyRuleRequest) (*meshes.ApplyRuleResponse, error) {
	...
	switch arReq.OpName {
	case installmTLSnsx-smCommand:
		installWithmTLS = true
		fallthrough
	case installnsx-smCommand:
		go func() {
			opName1 := "deploying"
			if arReq.DeleteOp {
				opName1 = "removing"
			...
			if err := iClient.executeInstall(ctx, installWithmTLS, arReq); err != nil {
				iClient.eventChan <- &meshes.EventsResponse{
					...
					Summary:     fmt.Sprintf("Error while %s nsx-sm", opName1),
					Details:     err.Error(),
				}
				return
			}
			...
			iClient.eventChan <- &meshes.EventsResponse{
				...
				Summary:     fmt.Sprintf("nsx-sm %s successfully", opName),
				Details:     fmt.Sprintf("The latest version of nsx-sm is now %s.", opName),
			}
			return
		}()
	...
	case installBookInfoCommand:
		go func() {
			...
			iClient.eventChan <- &meshes.EventsResponse{
				...
				Details:     fmt.Sprintf("The nsx-sm canonical Book Info app is now %s.", opName),
			}
			return
		}()
		return &meshes.ApplyRuleResponse{
